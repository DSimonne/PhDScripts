...
def Gas7Plot(self, filenameXCAT, filenameRGA, masslist, channelList,TimeRange=None):   
    ...
    for el in channelpos:
        plt.plot(GasLog[el][:,0], GasLog[el][:,1], colors[el], label = channels[el])
        yy=np.concatenate([yy, GasLog[el][:,1]])
    ...
    plt.subplot(712) #### Plot 2
    '''2) Flow setpoints
    '''
    yy=np.array([])
    for el in channelpos:
        plt.plot(GasLog[el][:,0], GasLog[el][:,2], colors[el], label = channels[el])
        yy=np.concatenate([yy, GasLog[el][:,2]])
    ...
    plt.subplot(713)  #### Plot 3
    '''3) Valves Positions
    '''
    yy=np.array([])
    #print channelpos
    for el in channelpos:
        plt.plot(GasLog[el][:,0], GasLog[el][:,3], colors[el], label = channels[el])
        yy=np.concatenate([yy, GasLog[el][:,3]])
