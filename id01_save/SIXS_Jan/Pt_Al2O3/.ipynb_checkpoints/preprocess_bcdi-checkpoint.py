...
scans = 1305  # np.arange(1401, 1419+1, 3)  # scan number or list of scan numbers
# scans = np.concatenate((scans, np.arange(1147, 1195+1, 3)))
# bad_indices = np.argwhere(scans == 738)
# scans = np.delete(scans, bad_indices)

root_folder = "/home/esrf/simonne/Documents/id01_david/SIXS_Jan/Pt_Al2O3/"  # folder of the experiment, where all scans are stored
save_dir = "/home/esrf/simonne/Documents/id01_david/SIXS_Jan/Pt_Al2O3/1305"  # images will be saved here, leave it to None otherwise
# (default to scan_folder/pynx/ or scan_folder/pynxraw/ depending on the setting of use_rawdata)
sample_name = ""  # str or list of str of sample names (string in front of the scan number in the folder name).
...
specfile_name = root_folder + 'analysis/alias_dict_2021.txt'
...
x_bragg = None #1 350  # horizontal pixel number of the Bragg peak, can be used for the definition of the ROI
y_bragg = None # 850  # vertical pixel number of the Bragg peak, can be used for the definition of the ROI
roi_detector = [100, 515, 250, 515] #[0, 515, 0, 515]  # [y_bragg - 200, y_bragg + 200, x_bragg - 200, x_bragg + 200]
...
background_file = root_folder + "analysis/mask_merlin.npy"  # root_folder + 'background.npz'  # non empty file path or None
...
#######################################################################################################
outofplane_angle = "gamma"  # out of plane detector angle (typically delta) in deg, corrected for the direct beam position.
# Leave None to use the uncorrected position.
inplane_angle = "delta"  # inplane detector angle (typically gamma) in deg, corrected for the direct beam position.
# Leave None to use the uncorrected position.
#########################################################################
