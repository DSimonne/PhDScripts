...
"""Python script to mvoe file from datadir to folder where it will be used by preprocess.bcdi"""
...
for scan in scans:
    ...
    try:
        filename = glob.glob(f"data/*mu*{scan}*")[0]
        shutil.copy2(filename, f"{folder}/S{scan}/data")
        print(f"Copîed {filename} to {folder}/S{scan}/data")
    except FileExistsError:
