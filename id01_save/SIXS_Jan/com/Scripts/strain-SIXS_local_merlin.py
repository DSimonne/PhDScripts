...
if sys.argv[2].startswith("["):
    scan = ast.literal_eval(sys.argv[2])
    
elif sys.argv[2]=="all":
    subdirnames = [x[1] for x in os.walk(f"{folder}/")][0]
    scan = [s.replace("S", "") for s in sorted(subdirnames) if s.startswith("S")]
    print(scan)
...
outofplane_angle = 0.326  # detector angle in deg (rotation around x outboard): delta ID01, delta SIXS, gamma 34ID
inplane_angle = 37.938  # detector angle in deg(rotation around y vertical up): nu ID01, gamma SIXS, tth 34ID
...
template_imagefile = 'CohCom_ascan_mu_%05d_R.nxs'
...
############################################
align_q = False  # if True rotates the crystal to align q along one axis of the array
...
axis_to_align = np.array([0, 1, 0]) # axis to align with ref_axis in the order x y z (axis 2, axis 1, axis 0)
...
# obj, extension = util.load_file("/users/simonne/Documents/id01_david/SIXS_Jan/Pt_Al2O3/" + sys.argv[1] + "/pynxraw/modes.h5")
