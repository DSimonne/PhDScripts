...
scans = 1414  # np.arange(1401, 1419+1, 3)  # scan number or list of scan numbers
# scans = np.concatenate((scans, np.arange(1147, 1195+1, 3)))
# bad_indices = np.argwhere(scans == 738)
# scans = np.delete(scans, bad_indices)

root_folder = "/home/david/Documents/PhD_local/PhDScripts/Pt_p2/5_CO_45_Ar/"  # folder of the experiment, where all scans are stored
...
medfilt_order = 5    # for custom median filter, number of pixels with intensity surrounding the empty pixel
#################################################
...
x_bragg = 160 #1 350  # horizontal pixel number of the Bragg peak, can be used for the definition of the ROI
y_bragg = 325 # 850  # vertical pixel number of the Bragg peak, can be used for the definition of the ROI
roi_detector = [0, 303, 0, 296] #  # [y_bragg - 200, y_bragg + 200, x_bragg - 200, x_bragg + 200]
