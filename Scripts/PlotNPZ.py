...
def TwoDimProj(file, axplot, datapath):
	"""For .npz files"""
	...
	@interact(
		...
		PlottingOptions = widgets.ToggleButtons(
			options = [("2D plot", "2D"), ("2D contour plot", "2DC"),("3D plot", "3D")],
		    value = "2D",
	...
	def PickLastAxis(i, PlottingOptions):
		...
		if PlottingOptions == "2D":
			...
			X, Y = np.meshgrid(np.linspace(0, dt.shape[0], dt.shape[0]), np.linspace(0, dt.shape[1], dt.shape[1]))
...
WidgetTwoDimProj = interactive(TwoDimProj,
